...
R.isNotEmpty = R.compose(R.not, R.isEmpty)

var _slice = Array.prototype.slice;
R.ifThen = R.curry(function (predicate, onTrue, value) {
    if (predicate(value)) {
        return onTrue(value);
    }
});

//calls fn on each node of the tree using getChildren fn for getting child nodes
R.traverseTree = R.curry(function (fn, getChildren, tree) {
    tree.map(function (node) {
        fn(node);
        R.traverseTree(fn, getChildren, getChildren(node) || []);
    });
    return tree;
});

R.sumObjFields = R.curry(function (fields, objs) {
    return R.reduce(function (acc, obj) {
        return R.mapArrayIntoObj(function (f) {
            var r = {};
            r[f] = acc[f] + obj[f];
            return r;
        }, fields);
    }, R.zipObj(fields, R.repeat(0, fields.length)), objs);
});

R.mapObjIntoArray = R.curry(function (fn, obj) {
    var result = [];
    R.mapObjIndexed(function (value, key) {
        result = result.concat(fn(value, key));
    }, obj);

    return result;
});

R.mapArrayIntoObj = R.curry(function (fn, list) {
    return R.compose(R.mergeAll, R.map(fn))(list);
});

R.pluckPath = R.curry(function (path, list) {
    var path = R.isArrayLike(path) ? path : path.split('.');
    return R.map(R.path(path), list);
});

R.delayed = R.curry(function (millis, fn) {
    return function () {
        var args = arguments;
        setTimeout(function () {
            fn.apply(this, args);
...
R.createBuffered = R.curry(function (millis, fn) {
    var currentTimeout = undefined;
    return function () {
        clearTimeout(currentTimeout);
        currentTimeout = setTimeout(fn.bind.apply(fn, [this].concat(_slice.call(arguments))), millis);
        return {
...
R.containsAny = R.curry(function (soughtList, sourceList) {
    if (R.is(String, sourceList)) {
        return R.any(R.stringContains(R.__, sourceList), soughtList);
    } else {
        return !!R.intersection(soughtList, sourceList).length;
    }
});

R.notContains = R.curryN(2, R.compose(R.not, R.contains));

R.isOneOf = R.flip(R.contains);

R.isNotNil = R.compose(R.not, R.isNil);

R.invoke = R.invoker(0);

R.copyArray = function (arr) {
    return arr.concat([]);
};

R.invokeEquals = R.curry(function (methodName, value, obj) {
    return obj[methodName]() === value;
});

R.invokeEq = R.invokeEquals;

R.removeEl = R.curry(function (value, list) {
    return R.remove(list.indexOf(value), 1, list);
});

R.switchString = R.curry(function (casesMap, value) {
    return casesMap[value](value);
});

R.stringContains = R.curry(function (searchStr, sourceString) {
    return sourceString.indexOf(searchStr) != -1;
});

/*
    returns the result of the first function which predicate have returned false
    Kind of the same as R.cond but looks for the false result.

    conditions = [[predicateFn, fn] [...] ... ]

    example:
        R.firstFalsy([
            [R.equals(5), (number) => console.log("Number is NOT equal five")]
        ], 6)
        will print "Number is NOT equal five"

    Useful for validations: R.firstFalsy
*/
/*R.firstFalsy = R.curry(function(conditions, value) // not sure if it is needed
{
    return
});
*/
 ...
 */
R.setPath = R.curry(function (path, value, object) {
    if (object == null) {
        return object;
    }
...
R.pickOrDefaults = R.curry(function (defaults, data) {
    return R.compose(R.merge(defaults), R.pick(R.__, data), R.keys)(defaults);
});

R.mapJoin = R.curry(function (mapper, glue, items) {
    return R.map(mapper, items).join(glue);
});

R.mapIndexed = R.curry(function (mapper, items) {
    return items.map(mapper);
});

/**
 * Returns items array with item if it doesn't contain item otherwise original items array
 */
R.ensureContains = R.curry(function (item, items) {
    return items.indexOf(item) === -1 ? [].concat(items, [item]) : items;
});

R.rejectObj = R.curry(function (fn, obj) {
    return R.compose(R.fromPairs(), R.reject(function (_ref) {
        var key = _ref[0];
        var val = _ref[1];
...
R.propDefined = R.curry(function (prop, obj) {
    return obj[prop] !== undefined && obj[prop] !== null;
});

R.isOneOfBy = R.curryN(3, function(soughtList, fn, obj)
{
    return R.compose(
        R.isOneOf(soughtList),
        fn
    )(obj);
});

R.isTruthy = function (x) {
    return !!x;
};

R.clean = R.filter(R.isTruthy);

/**
 * Escapes a string to use it with RegExp constructor
 * @function
 * @param {string} str
 * @return {string}
 */
R.escapeRegExp = R.compose(R.replace(/[.?*+^$[\]\\(){}|-]/g, '\\$&'), String);

R.propNotEq = R.curryN(3, R.compose(R.not, R.propEq));

R.pathNotEq = R.curryN(3, R.compose(R.not, R.pathEq));

R.startsWith = R.curry(function (start, str) {
    return str.indexOf(start) === 0;
});
R.endsWith = R.curry(function (end, str) {
    return str.substr(str.length - end.length) === end;
});

R.notEq = R.curryN(2, R.compose(R.not, R.equals));

R.deepMerge = R.curry(function (target, src) {
        ...
        Object.keys(src).forEach(function (key) {
            if (typeof src[key] !== 'object' || !src[key]) {
                ...
                if (!target[key]) {
                    ...
                    dst[key] = R.deepMerge(target[key], src[key]);
                }
            }
        });
    }
...
//(a -> bool) -> [(b) -> a] -> b -> a
R.findResult = R.curryN(3,
    function (pred, fnList, arg)
    {
        var i = 0;
        var res = null;
...
// templateStr - example: "here is a $obj_prop value from input obj"
// obj - Object to take props from
R.template = R.curryN(2, function (templateStr, obj) {
    return R.reduce(function (acc, key_val) {
        return acc.replace('$' + key_val[0], key_val[1]);
    }, templateStr, R.toPairs(obj));
});

R.ensureList = R.ifElse(
    R.is(Array),
    R.identity,
    R.of
);

R.ensureArray = R.ensureList;
(function(){var s,t,p,n,q,r=[].slice;window.iced={Deferrals:function(){function a(a){this.continuation=a;this.count=1;this.ret=null}a.prototype._fulfill=function(){if(!--this.count)return this.continuation(this.ret)};a.prototype.defer=function(a){var c=this;++this.count;return function(){var d,e;d=1<=arguments.length?r.call(arguments,0):[];null!=a&&null!=(e=a.assign_fn)&&e.apply(null,d);return c._fulfill()}};return a}(),findDeferral:function(){return null},trampoline:function(a){return a()}};window.__iced_k=
window.__iced_k_noop=function(){};this.ns=function(a){var b,c,d,e;b=a.split(".");a=window[b[0]];void 0===a&&(a=window[b[0]]={});c=b.slice(1);d=0;for(e=c.length;d<e;d++)b=c[d],a=a[b]=a[b]||{};return a};this.ultimello={};this.ultimello.utils={ns:this.ns,mixin:function(a,b){for(var c in b)a[c]=b[c];return a},toArray:function(a){var b;b=a;a.constructor!==Array&&(b=[a]);return b},getBufferedFn:function(a,b){var c,d;d=null;c=function(){d=null;return a.apply(this,arguments)};return function(){var a;a=1<=
arguments.length?r.call(arguments,0):[];null!=d&&(clearTimeout(d),d=null);return d=setTimeout(function(){return c.apply(this,a)},b)}},wait_for:function(a,b){var c,d,e,f,g,h;f=a.predicate;d=a.interval;g=a.timeout;c=h=!1;g&&setTimeout(function(){return h=!0},g);return e=setInterval(function(){var a;if(((a=f())||h)&&!c)return c=!0,clearInterval(e),"function"===typeof b?b(a):void 0},d)},wait_for_async:function(a,b){var c,d,e,f,g,h;f=a.predicate;e=a.interval;g=a.timeout;c=h=!1;g&&setTimeout(function(){return h=
!0},g);d=function(){var a,g,k;g=iced.findDeferral(arguments);h?"function"===typeof b&&b(!1):function(b){k=new iced.Deferrals(b,{parent:g,funcname:"fn"});f(k.defer({assign_fn:function(){return function(b){return a=b}}(),lineno:80}));k._fulfill()}(function(){return(a||h)&&!c?(c=!0,"function"===typeof b?b(a):void 0):setTimeout(d,e)})};return d()},wait_until:function(a,b,c){var d,e,f,g,h,l,m,k;m=iced.findDeferral(arguments);e=a.predicate;l=a.transform;d=a.interval;h=a.timeout;f=b;(function(a){k=new iced.Deferrals(a,
{parent:m,funcname:"wait_until"});ultimello.utils.wait_for({predicate:function(){f=l(f);return e(f)},interval:d,timeout:h},k.defer({assign_fn:function(){return function(a){return g=a}}(),lineno:96}));k._fulfill()})(function(){return"function"===typeof c?c(g?f:!1):void 0})},wait_until_async:function(a,b,c){var d,e,f,g,h,l,m,k;m=iced.findDeferral(arguments);e=a.predicate;l=a.transform;d=a.interval;h=a.timeout;f=b;(function(a){k=new iced.Deferrals(a,{parent:m,funcname:"wait_until_async"});ultimello.utils.wait_for_async({predicate:function(a){var b,
c,d;c=iced.findDeferral(arguments);(function(a){d=new iced.Deferrals(a,{parent:c,funcname:"predicate"});l(b,d.defer({assign_fn:function(){return function(a){return b=a}}(),lineno:104}));d._fulfill()})(function(){return e(b,a)})},interval:d,timeout:h},k.defer({assign_fn:function(){return function(a){return g=a}}(),lineno:108}));k._fulfill()})(function(){return"function"===typeof c?c(g?f:!1):void 0})},mutationRecordsHasElement:function(a,b){var c=this;return a.some(function(a){return c.mutationRecordHasElement(a,
b)})},mutationRecordHasElement:function(a,b){return["addedNodes","removedNodes"].some(function(c){return $.makeArray(a[c]).some(function(a){return $(a).hasClass(b)})})},orderElementsAsInList:function(a,b){var c,d,e,f,g,h,l,m;f=[];l=a.slice(0,-1);d=g=0;for(h=l.length;g<h;d=++g)if(c=l[d],c=b(c),null!=c&&null!=(m=c.parentNode)&&m.tagName)e=b(a[d+1]),c&&e&&($(e).insertAfter(c),f[d]=c,f[d+1]=e);return f},getCardShortIdAndUrlFromEl:function(a){var b;(b=$(a).attr("href")||$(a).find(".js-card-name").attr("href"))?
(a=+b.split("/").last().split("-")[0],b=ultimello.utils.getShortUrlForShortLink(b.split("/")[2])):b=a=null;return{shortId:a,shortUrl:b}},getShortUrlForShortLink:function(a){return"https://trello.com/c/"+a},getShortUrlFromUrl:function(a){return"https://trello.com/c/"+a.split("https://trello.com/c/")[1].split("/")[0]},getCardElForShortUrl:function(a){a=R.last(a.split("/"));return $(".list-card[href*='/"+a+"/']")[0]},isCardShortUrl:function(a){return a.startsWith("https://trello.com/c/")},parseTrelloUrl:function(a){var b,
c;c=a.replace("//","/").split("/");b=c[2];a=c[3];5<=c.length&&r.call(c,4);a:switch(b){case "c":b="trello-card-opened";break a;default:b=null}return{state:b,cardShortLink:a}},openTab:function(a){return chrome.tabs.create({url:chrome.extension.getURL(a)})},autoBindAll:function(a){for(var b in a)R.is(Function,a[b])&&(a[b]=a[b].bind(a));return a},get_move_operations_to_equalize_list:function(a,b){return R.until(function(c){return R.equals(c.list,a)||c.next_item_index===b.length-1},function(c){var d,e;
e=c;c.list[c.next_item_index]!==a[c.next_item_index]&&(d=a[c.next_item_index],e=Object.assign(e,{operations:R.append({new_value:d,new_pos:c.next_item_index},c.operations),list:ultimello.utils.moveItemInArray(b.indexOf(d),c.next_item_index,c.list)}));e.next_item_index=c.next_item_index+1;return e},{operations:[],list:b,next_item_index:0}).operations},moveItemInArray:function(a,b,c){c.splice(b,0,c.splice(a,1)[0]);return c},parseResponseHeaders:function(a){var b,c,d,e,f,g;c={};if(!a)return c;a=a.split("\r\n");
d=f=0;for(g=a.length;f<g;d=f+=1)b=a[d],e=b.indexOf(": "),0<e&&(d=b.substring(0,e),b=b.substring(e+2),c[d]=b);return c},send_log:function(a){return $.rest_ajax.post("https://ultimello.com:440/l",a)}};ultimello.utils.ns("ultimello.utils");ultimello.utils.storage={load:function(a){a=localStorage[this._key(a)];return null!=a?JSON.parse(a):null},save:function(a,b){var c;c=this._key(a);return null==b?delete localStorage[c]:localStorage[c]=JSON.stringify(b)},_key:function(a){return"ultimello_"+a}};n=ultimello.utils;
s={storage:n.storage};chrome.runtime.onMessage.addListener(function(a,b,c){b=s[a.facility];a=null!=b?b[a.method].apply(b,a.params||[]):null;return c(a)});chrome.browserAction.onClicked.addListener(function(a){return n.openTab("settings.html")});q=chrome.runtime.getManifest().version;p=n.storage.load("installed-version");(t=!p)&&n.openTab("settings.html");!t&&p!==q&&n.storage.save("previous-version",p);n.storage.save("installed-version",q);chrome.runtime.setUninstallURL&&chrome.runtime.setUninstallURL("http://ultimello.com/uninstall-form.html")}).call(this);
